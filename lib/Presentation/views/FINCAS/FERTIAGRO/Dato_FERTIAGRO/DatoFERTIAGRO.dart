...
class _DatoFERTIAGRO_ScreenState extends State<DatoFERTIAGRO_Screen> {
  bool _showResults_FERTIAGRO = false;
  final ScrollController _scrollController_FERTIAGRO = ScrollController();
  ...
  final DatabaseReference _FERTIAGRORef =
  ...
  void _addListeners() {
    ...
    _RendimientoController.addListener(_updateCalculations);
  }
  ...
  void _addData() {
    ...
    setState(() {
      _showResults_FERTIAGRO = !_showResults_FERTIAGRO;
    });

    Future.delayed(const Duration(milliseconds: 500), () {
      if (_showResults_FERTIAGRO) {
        _scrollController_FERTIAGRO.animateTo(
          _scrollController_FERTIAGRO.position.maxScrollExtent,
...
// Función para mostrar mensajes con SnackBar
  ...
  String get tutorial_mostrado_FERTIAGRO_DATOS =>
      "tutorial_mostrado_FERTIAGRO_DATOS";

  // Verifica si el tutorial ya fue mostrado
  Future<bool> _shouldShowTutorialDATO() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(tutorial_mostrado_FERTIAGRO_DATOS) ??
...
// Guarda que el tutorial ya se mostró
  Future<void> _setTutorialShownDATO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_FERTIAGRO_DATOS, false);
  }

// Restablece el tutorial desde ajustes si el usuario lo quiere repetir
  Future<void> resetTutorialDATO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_FERTIAGRO_DATOS, true);
  }
  ...
  void initState() {
    ...
    _kgXHaController.addListener(() {
      setState(() {
        selectedGramos = _kgXHaController.text;
      });
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        controller: _scrollController_FERTIAGRO,
        child: Padding(
