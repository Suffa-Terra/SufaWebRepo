...
class _DatoGROVITAL_ScreenState extends State<DatoGROVITAL_Screen> {
  bool _showResults_GROVITAL = false;
  final ScrollController _scrollController_GROVITAL = ScrollController();
  ...
  void _addData() {
    ...
    setState(() {
      _showResults_GROVITAL = !_showResults_GROVITAL;
    });

    Future.delayed(const Duration(milliseconds: 500), () {
      if (_showResults_GROVITAL) {
        _scrollController_GROVITAL.animateTo(
          _scrollController_GROVITAL.position.maxScrollExtent,
...
// Función para mostrar mensajes con SnackBar
  ...
  String get tutorial_mostrado_GROVITAL_DATOS =>
      "tutorial_mostrado_GROVITAL_DATOS";

  // Verifica si el tutorial ya fue mostrado
  Future<bool> _shouldShowTutorialDATO() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(tutorial_mostrado_GROVITAL_DATOS) ??
...
// Guarda que el tutorial ya se mostró
  Future<void> _setTutorialShownDATO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_GROVITAL_DATOS, false);
  }

// Restablece el tutorial desde ajustes si el usuario lo quiere repetir
  Future<void> resetTutorialDATO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_GROVITAL_DATOS, true);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        controller: _scrollController_GROVITAL,
        child: Padding(
