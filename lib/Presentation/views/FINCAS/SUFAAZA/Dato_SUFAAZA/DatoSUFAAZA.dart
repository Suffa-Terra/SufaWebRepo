...
class _DatoSUFAAZA_ScreenState extends State<DatoSUFAAZA_Screen> {
  bool _showResults_SUFAAZA = false;
  final ScrollController _scrollController_SUFAAZA = ScrollController();
  ...
  void _addData() {
    ...
    setState(() {
      _showResults_SUFAAZA = !_showResults_SUFAAZA;
    });

    Future.delayed(const Duration(milliseconds: 500), () {
      if (_showResults_SUFAAZA) {
        _scrollController_SUFAAZA.animateTo(
          _scrollController_SUFAAZA.position.maxScrollExtent,
...
// Función para mostrar mensajes con SnackBar
  ...
  String get tutorial_mostrado_SUFAAZA_DATOS =>
      "tutorial_mostrado_SUFAAZA_DATOS";

  // Verifica si el tutorial ya fue mostrado
  Future<bool> _shouldShowTutorialDATO() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(tutorial_mostrado_SUFAAZA_DATOS) ??
...
// Guarda que el tutorial ya se mostró
  Future<void> _setTutorialShownDATO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_SUFAAZA_DATOS, false);
  }

// Restablece el tutorial desde ajustes si el usuario lo quiere repetir
  Future<void> resetTutorialDATO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_SUFAAZA_DATOS, true);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        controller: _scrollController_SUFAAZA,
        child: Padding(
        ...
        Wrap(
          ...
          children: pagedItems.map((itemHect) {
            ...
            return FilterChip(
              ...
              backgroundColor: const Color(0xffe2d5d5),
