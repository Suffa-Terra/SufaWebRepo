...
class _DatoTIERRAVID_ScreenState extends State<DatoTIERRAVID_Screen> {
  bool _showResults_TIERRAVID = false;
  final ScrollController _scrollController_TIERRAVID = ScrollController();
  ...
  void _addData() {
    ...
    setState(() {
      _showResults_TIERRAVID = !_showResults_TIERRAVID;
    });

    Future.delayed(const Duration(milliseconds: 500), () {
      if (_showResults_TIERRAVID) {
        _scrollController_TIERRAVID.animateTo(
          _scrollController_TIERRAVID.position.maxScrollExtent,
...
// Función para mostrar mensajes con SnackBar
  ...
  String get tutorial_mostrado_TIERRAVID_DATOS =>
      "tutorial_mostrado_TIERRAVID_DATOS";

  // Verifica si el tutorial ya fue mostrado
  Future<bool> _shouldShowTutorialDATO() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(tutorial_mostrado_TIERRAVID_DATOS) ??
...
// Guarda que el tutorial ya se mostró
  Future<void> _setTutorialShownDATO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_TIERRAVID_DATOS, false);
  }

// Restablece el tutorial desde ajustes si el usuario lo quiere repetir
  Future<void> resetTutorialDATO() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(tutorial_mostrado_TIERRAVID_DATOS, true);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        controller: _scrollController_TIERRAVID,
        child: Padding(
        ...
        Wrap(
          ...
          children: pagedItems.map((itemHect) {
            ...
            return FilterChip(
              ...
              backgroundColor: const Color(0xffe2dfd7),
